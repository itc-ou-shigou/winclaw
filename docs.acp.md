...
This document describes how the WinClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`winclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
WinClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `winclaw acp` and speaks ACP messages over stdio.
...
`winclaw acp` resolves the Gateway URL and auth from CLI flags or config:
