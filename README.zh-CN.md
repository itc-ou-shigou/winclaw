...
1. 从 [SourceForge](https://sourceforge.net/projects/winclaw/files/WinClawSetup-2026.3.6.exe/download) 或 [GitHub Releases](https://github.com/itc-ou-shigou/winclaw/releases/latest) 下载 `WinClawSetup-{version}.exe`（也可从本仓库 [`releases/`](releases/) 目录获取）
...
2. **回答问题** — WinClaw 检测项目类型后，询问预算、流量、数据库、安全需求
3. **安全设计** *（按需）* — 企业级需求时，自动调用 `/architecture` 设计 WAF、KMS、IAM、威胁检测、合规架构
4. **审核方案** — WinClaw 推荐架构并展示费用明细，确认或调整
5. **执行部署** — WinClaw 生成基础设施模板，创建云资源，部署应用代码
6. **自动验证与修复** — 执行健康检查，发现错误时自动诊断、修复并重新部署，直到全部通过。仅在需要人工操作时请求用户简单协助
