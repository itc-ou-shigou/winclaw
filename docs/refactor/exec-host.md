...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.winclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.winclaw/node.json` (node id + pairing token).
...
Path: `~/.winclaw/exec-approvals.json`
...
- Unix socket at `~/.winclaw/exec-approvals.sock` (0600).
