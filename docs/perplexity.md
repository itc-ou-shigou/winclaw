...
WinClaw uses Perplexity Search API for the `web_search` tool when `provider: "perplexity"` is set.
...
`~/.winclaw/winclaw.json` under `tools.web.search.perplexity.apiKey`.
