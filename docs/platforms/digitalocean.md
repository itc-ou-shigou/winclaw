---
summary: "WinClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up WinClaw on DigitalOcean
  - Looking for cheap VPS hosting for WinClaw
title: "DigitalOcean"
---

# WinClaw on DigitalOcean

## Goal

Run a persistent WinClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status winclaw-gateway.service

# View logs
journalctl --user -u winclaw-gateway.service -f
```
...
```bash
winclaw channels login whatsapp
# Scan QR code
```
...
- `~/.winclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u winclaw --no-pager -n 50
```
