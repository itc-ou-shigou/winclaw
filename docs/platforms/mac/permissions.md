...
- Same path: run the app from a fixed location (for WinClaw, `dist/WinClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.winclaw.mac
sudo tccutil reset ScreenCapture ai.winclaw.mac
...
Workaround: move files into the WinClaw workspace (`~/.winclaw/workspace`) if you want to avoid per-folder grants.
