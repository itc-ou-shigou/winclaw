...
service via the external `winclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.winclaw.gateway`
  (or `ai.winclaw.<profile>` when using `--profile`/`WINCLAW_PROFILE`; legacy `com.winclaw.*` is supported).
