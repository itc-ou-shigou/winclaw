---
summary: "WinClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up WinClaw on a Raspberry Pi
  - Running WinClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# WinClaw on Raspberry Pi

## Goal

Run a persistent, always-on WinClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
winclaw config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/winclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/winclaw-compile-cache
mkdir -p /var/tmp/winclaw-compile-cache
export WINCLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `WINCLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running WinClaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit winclaw
```

```ini
[Service]
Environment=WINCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/winclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep WinClaw state/cache on SSD-backed storage to avoid SD-card
...
Most WinClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled winclaw

# Enable if not
sudo systemctl enable winclaw

# Start on boot
sudo systemctl start winclaw
```
...
```bash
# Check logs
journalctl -u winclaw --no-pager -n 100

# Common fix: rebuild
cd ~/winclaw  # if using hackable install
npm run build
sudo systemctl restart winclaw
```
