...
| `$WINCLAW_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys)                             |
| `$WINCLAW_STATE_DIR/agents/<agentId>/agent/auth.json`          | Runtime auth cache (managed automatically)                   |
...
Do **not** commit anything under `~/.winclaw` (credentials, sessions, tokens).
...
Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.winclaw.*`, `winclaw-gateway-<profile>.service`, `WinClaw Gateway (<profile>)`).
...
- Default behavior on DM-capable channels is **pairing**:
  ...
  - Approve with: `winclaw pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `winclaw pairing list <channel>` if a code didn't arrive.
