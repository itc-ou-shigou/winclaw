---
summary: "Where WinClaw loads environment variables and the precedence order"
read_when:
...
WinClaw pulls environment variables from multiple sources. The rule is **never override existing values**.

## Precedence (highest → lowest)

1. **Process environment** (what the Gateway process already has from the parent shell/daemon).
2. **`.env` in the current working directory** (dotenv default; does not override).
3. **Global `.env`** at `~/.winclaw/.env` (aka `$WINCLAW_STATE_DIR/.env`; does not override).
4. **Config `env` block** in `~/.winclaw/winclaw.json` (applied only if missing).
5. **Optional login-shell import** (`env.shellEnv.enabled` or `WINCLAW_LOAD_SHELL_ENV=1`), applied only for missing expected keys.
...
WinClaw supports two env-driven patterns:
...
**Precedence:** `WINCLAW_HOME` > `$HOME` > `USERPROFILE` > `os.homedir()`
...
<dict>
  <key>WINCLAW_HOME</key>
  <string>/Users/kira</string>
</dict>
```

`WINCLAW_HOME` can also be set to a tilde path (e.g. `~/svc`), which gets expanded using `$HOME` before use.
