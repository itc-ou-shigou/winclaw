---
summary: "Use OpenAI via API keys or Codex subscription in WinClaw"
read_when:
  - You want to use OpenAI models in WinClaw
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like WinClaw.
...
```bash
winclaw onboard --auth-choice openai-api-key
# or non-interactive
winclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
winclaw onboard --auth-choice openai-codex

# Or run OAuth directly
winclaw models auth login --provider openai-codex
```
...
WinClaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), WinClaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. WinClaw enables it by default for
...
`baseUrl` on `api.openai.com`), WinClaw now auto-enables OpenAI server-side
