---
summary: "Use MiniMax M2.5 in WinClaw"
read_when:
  - You want MiniMax models in WinClaw
...
- **Compatibility:** WinClaw still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
winclaw plugins enable minimax-portal-auth  # skip if already loaded.
winclaw gateway restart  # restart if gateway is already running
winclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/winclaw/winclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `winclaw.json`:
...
- Use `winclaw models list` and `winclaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `winclaw configure` and selecting **MiniMax M2.5**, or
