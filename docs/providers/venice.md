---
summary: "Use Venice AI privacy-focused models in WinClaw"
read_when:
  - You want privacy-focused inference in WinClaw
...
```bash
winclaw onboard --auth-choice venice-api-key
```
...
After setup, WinClaw shows all available Venice models. Pick based on your needs:
...
```bash
winclaw models set venice/claude-opus-45
winclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
winclaw models list | grep venice
```

## Configure via `winclaw configure`

1. Run `winclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
WinClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
The Venice model catalog updates dynamically. Run `winclaw models list` to see currently available models. Some models may be temporarily offline.
