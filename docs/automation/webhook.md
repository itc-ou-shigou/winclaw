...
read_when:
  ...
  - Wiring external systems into WinClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your WinClaw config directory (typically `~/.winclaw/hooks/transforms`).
...
- `winclaw webhooks gmail setup` writes `hooks.gmail` config for `winclaw webhooks gmail run`.
