...
This plan defines how WinClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- WinClaw core owns ACP control plane concerns
...
WinClaw should not reimplement ACP transport internals in core.
WinClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in WinClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under WinClaw state dir
...
- ACP event stream is normalized to WinClaw reply chunks
...
For the first production backend (`extensions/acpx`), WinClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses WinClaw `sessionKey`)
...
- WinClaw consumes ndjson events from `acpx --format json --json-strict`
