...
read_when:
  - Designing workspace memory (~/.winclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep WinClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.winclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.winclaw/workspace` as canonical human-readable memory.
...
```
~/.winclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in WinClaw**, but keep a separable core library.

### Why integrate into WinClaw?

- WinClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `winclaw memory recall "…" --k 25 --since 30d`
  - `winclaw memory reflect --since 7d`
...
Pragmatic take for `~/.winclaw/workspace`:
