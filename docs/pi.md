...
This document describes how WinClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

WinClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, WinClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in WinClaw's local TUI mode)                                              |
...
├── pi-tools.ts                    # createWinClawCodingTools()
...
├── winclaw-tools.ts              # WinClaw-specific tools
...
2. **Custom Replacements**: WinClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **WinClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures WinClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, WinClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
WinClaw wraps this with `guardSessionManager()` for tool result safety.
...
WinClaw maintains an auth profile store with multiple API keys per provider:
...
WinClaw loads custom pi extensions for specialized behavior:
...
WinClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | WinClaw Embedded                                                                             |
| --------------- | ----------------------- | -------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                               |
| Tools           | Default coding tools    | Custom WinClaw tool suite                                                                    |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                  |
| Session storage | `~/.pi/agent/sessions/` | `~/.winclaw/agents/<agentId>/sessions/` (or `$WINCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-winclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-winclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-winclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-winclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
