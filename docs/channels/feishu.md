...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects WinClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
winclaw plugins install @winclaw/feishu
```

Local checkout (when running from a git repo):

```bash
winclaw plugins install ./extensions/feishu
```
...
If you just installed WinClaw, run the wizard:
...
2. Configuring app credentials in WinClaw
...
1. You already ran `winclaw channels add` for Feishu
2. The gateway is running (`winclaw gateway status`)
...
```bash
winclaw pairing approve feishu <CODE>
```
...
2. Run `winclaw logs --follow` and look for `chat_id`
...
2. Run `winclaw logs --follow` and look for `open_id`
...
| `winclaw gateway restart` | Restart gateway service       |
| `winclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `winclaw logs --follow`
...
5. Ensure the gateway is running: `winclaw gateway status`
6. Check logs: `winclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.winclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.winclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
