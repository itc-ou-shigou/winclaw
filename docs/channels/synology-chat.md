---
summary: "Synology Chat webhook setup and WinClaw config"
read_when:
  - Setting up Synology Chat with WinClaw
...
```bash
winclaw plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your WinClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in WinClaw.
...
- Pairing approvals work with:
  - `winclaw pairing list synology-chat`
  - `winclaw pairing approve synology-chat <CODE>`
...
```bash
winclaw message send --channel synology-chat --target 123456 --text "Hello from WinClaw"
winclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
