...
Tlon is a decentralized messenger built on Urbit. WinClaw connects to your Urbit ship and can
...
```bash
winclaw plugins install @winclaw/tlon
```

Local checkout (when running from a git repo):

```bash
winclaw plugins install ./extensions/tlon
```
...
By default, WinClaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `winclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, WinClaw replies in-thread.
