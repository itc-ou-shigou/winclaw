...
read_when:
  - Setting up Twitch chat integration for WinClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. WinClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
winclaw plugins install @winclaw/twitch
```

Local checkout (when running from a git repo):

```bash
winclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `WINCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "winclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use WINCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
WINCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
