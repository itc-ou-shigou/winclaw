...
Matrix is an open, decentralized messaging protocol. WinClaw connects as a Matrix **user**
...
```bash
winclaw plugins install @winclaw/matrix
```

Local checkout (when running from a git repo):

```bash
winclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `winclaw plugins install @winclaw/matrix`
   - From a local checkout: `winclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: WinClaw calls the same
     login endpoint, stores the access token in `~/.winclaw/credentials/matrix/credentials.json`,
...
- On first connection, WinClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  WinClaw logs a warning.
...
`~/.winclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `winclaw pairing list matrix`
  - `winclaw pairing approve matrix <CODE>`
...
- On startup, WinClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
