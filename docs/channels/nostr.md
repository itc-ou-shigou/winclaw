...
read_when:
  - You want WinClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables WinClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`winclaw onboard`) and `winclaw channels add` list optional channel plugins.
...
```bash
winclaw plugins install @winclaw/nostr
```

Use a local checkout (dev workflows):

```bash
winclaw plugins install --link <path-to-winclaw>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "winclaw@example.com",
        "lud16": "winclaw@example.com"
      }
    }
  }
}
