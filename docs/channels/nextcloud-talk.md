...
```bash
winclaw plugins install @winclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
winclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "WinClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `winclaw pairing list nextcloud-talk`
  - `winclaw pairing approve nextcloud-talk <CODE>`
