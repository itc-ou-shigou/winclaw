...
read_when:
  - Setting up Zalo Personal for WinClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside WinClaw.
...
- Install via CLI: `winclaw plugins install @winclaw/zalouser`
- Or from a source checkout: `winclaw plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `winclaw channels login --channel zalouser`
...
```bash
winclaw directory self --channel zalouser
winclaw directory peers list --channel zalouser --query "name"
winclaw directory groups list --channel zalouser --query "work"
```
...
- `winclaw pairing list zalouser`
- `winclaw pairing approve zalouser <code>`
...
- On startup, WinClaw resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in WinClaw state. Example:
...
- WinClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, WinClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `winclaw channels logout --channel zalouser && winclaw channels login --channel zalouser`
...
- The channel now runs fully in WinClaw without external CLI binaries.
