...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in WinClaw
...
Running WinClaw via Node with `tsx` fails at startup with:

```
[winclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `winclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch winclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node winclaw.mjs status` works on Node 25.
