---
summary: "How WinClaw builds prompt context and reports token usage + costs"
read_when:
...
WinClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

WinClaw assembles its own system prompt on every run. It includes:
...
For images, WinClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from WinClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `winclaw status --usage` and `winclaw channels list` show
...
`cacheWrite`. If pricing is missing, WinClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. WinClaw can
...
Anthropic's 1M context window is currently beta-gated. WinClaw can inject the
...
WinClaw skips the `context-1m-*` beta header because Anthropic currently
