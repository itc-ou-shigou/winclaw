...
WinClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- WinClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- WinClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
