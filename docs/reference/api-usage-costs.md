...
WinClaw features that can generate provider usage or paid API calls.
...
- `winclaw status --usage` and `winclaw channels list` show provider **usage windows**
...
WinClaw can pick up credentials from:
...
- `winclaw status --usage`
- `winclaw models status --json`
...
`winclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
