...
- With Anthropic API-key auth profiles, WinClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, WinClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
WinClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.winclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$WINCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `WINCLAW_CACHE_TRACE=1` enables cache tracing.
- `WINCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `WINCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `WINCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `WINCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
