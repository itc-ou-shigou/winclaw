...
- `/usage cost` in chats: local cost summary aggregated from WinClaw session logs.
- CLI: `winclaw status --usage` prints a full per-provider breakdown.
- CLI: `winclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
