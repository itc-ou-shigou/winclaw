...
- **gateway host** → `winclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.winclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.winclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `winclaw approvals allowlist ...`).
- `winclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `winclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.winclaw/exec-approvals.json` directly.

CLI: `winclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
