...
**Recommended:** run `winclaw configure --section web`. It stores the key in
`~/.winclaw/winclaw.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.winclaw/.env` (or your
service environment). See [Env vars](/help/faq#how-does-winclaw-load-environment-variables).
...
environment. For a gateway install, put it in `~/.winclaw/.env`.

If no base URL is set, WinClaw chooses a default based on the API key source:
