...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `winclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `winclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=winclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${WINCLAW_STATE_DIR:-$HOME/.winclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
