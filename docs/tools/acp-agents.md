...
```bash
winclaw plugins install @winclaw/acpx
...
`@winclaw/acpx` now enforces a strict plugin-local pinning model:
...
- WinClaw startup stays non-blocking while acpx ensure runs.
