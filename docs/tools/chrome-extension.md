---
summary: "Chrome extension: let WinClaw drive your existing Chrome tab"
read_when:
...
The WinClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate winclaw-managed Chrome profile.
...
WinClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the WinClaw release (npm package) as static files. There is no separate “build” step.

After upgrading WinClaw:

- Re-run `winclaw browser extension install` to refresh the installed files under your WinClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

WinClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `WINCLAW_GATEWAY_TOKEN`)
...
- Open the tab you want WinClaw to control.
...
- `ON`: attached; WinClaw can drive that tab.
...
Debugging: `winclaw sandbox explain`
...
`winclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `winclaw browser extension install` first to copy the extension to a stable location under your WinClaw state directory.
...
- **This is not isolated** like the dedicated winclaw-managed profile.
