---
summary: "Direct `winclaw agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `winclaw agent` (direct agent runs)

`winclaw agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `winclaw message --target`).
...
```bash
winclaw agent --to +15555550123 --message "status update"
...
winclaw agent --to +15555550123 --message "Summon reply" --deliver
