...
Deploy WinClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/winclaw` — Control UI
...
- `WINCLAW_STATE_DIR=/data/.winclaw` (recommended)
- `WINCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `WINCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your WinClaw state + workspace so you can migrate to another host without losing config or memory.
