...
Deploy WinClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/winclaw/winclaw)
...
repository configures everything needed to run WinClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: WINCLAW_STATE_DIR
        value: /data/.winclaw
      - key: WINCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: WINCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/winclaw`.
...
If you use the original WinClaw repository, Render will not auto-deploy your WinClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For WinClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any WinClaw host.
