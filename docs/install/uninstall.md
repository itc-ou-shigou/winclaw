...
Default label is `ai.winclaw.gateway` (or `ai.winclaw.<profile>`; legacy `com.winclaw.*` may still exist):

```bash
launchctl bootout gui/$UID/ai.winclaw.gateway
rm -f ~/Library/LaunchAgents/ai.winclaw.gateway.plist
```

If you used a profile, replace the label and plist name with `ai.winclaw.<profile>`. Remove any legacy `com.winclaw.*` plists if present.
