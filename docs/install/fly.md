---
title: Fly.io
description: Deploy WinClaw on Fly.io
summary: "Step-by-step Fly.io deployment for WinClaw with persistent storage and HTTPS"
read_when:
  - Deploying WinClaw on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** WinClaw Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-winclaw

# Create a persistent volume (1GB is usually enough)
fly volumes create winclaw_data --size 1 --region iad
```
...
[env]
  NODE_ENV = "production"
  WINCLAW_PREFER_PNPM = "1"
  WINCLAW_STATE_DIR = "/data"
...
fly secrets set WINCLAW_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `WINCLAW_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `winclaw.json` where they could be accidentally exposed or logged.
...
**Note:** With `WINCLAW_STATE_DIR=/data`, the config path is `/data/winclaw.json`.
...
Or visit `https://my-winclaw.fly.dev/`

Paste your gateway token (the one from `WINCLAW_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `WINCLAW_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/winclaw.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/winclaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/winclaw.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/winclaw.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/winclaw.json"
```
...
**Fix:** Ensure `WINCLAW_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-winclaw

# Release public IPs
fly ips release <public-ipv4> -a my-winclaw
fly ips release <public-ipv6> -a my-winclaw
...
fly ips allocate-v6 --private -a my-winclaw
```
...
fly proxy 3000:3000 -a my-winclaw
