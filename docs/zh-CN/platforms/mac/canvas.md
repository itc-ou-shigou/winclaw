...
- `~/Library/Application Support/WinClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `winclaw-canvas://<session>/<path>`

示例：

- `winclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `winclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `winclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
winclaw nodes canvas present --node <id>
winclaw nodes canvas navigate --node <id> --url "/"
winclaw nodes canvas eval --node <id> --js "document.title"
winclaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__winclaw__/a2ui/
```
...
winclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
winclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "winclaw://agent?message=Review%20this%20design";
```
