...
你可以在 `~/.winclaw/winclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.winclaw/winclaw.json` 的 `logging` 下。
...
- WinClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `winclaw plugins enable diagnostics-otel` 启用插件。
...
- `winclaw.tokens`（计数器，属性：`winclaw.token`、`winclaw.channel`、`winclaw.provider`、`winclaw.model`）
- `winclaw.cost.usd`（计数器，属性：`winclaw.channel`、`winclaw.provider`、`winclaw.model`）
- `winclaw.run.duration_ms`（直方图，属性：`winclaw.channel`、`winclaw.provider`、`winclaw.model`）
- `winclaw.context.tokens`（直方图，属性：`winclaw.context`、`winclaw.channel`、`winclaw.provider`、`winclaw.model`）

消息流：

- `winclaw.webhook.received`（计数器，属性：`winclaw.channel`、`winclaw.webhook`）
- `winclaw.webhook.error`（计数器，属性：`winclaw.channel`、`winclaw.webhook`）
- `winclaw.webhook.duration_ms`（直方图，属性：`winclaw.channel`、`winclaw.webhook`）
- `winclaw.message.queued`（计数器，属性：`winclaw.channel`、`winclaw.source`）
- `winclaw.message.processed`（计数器，属性：`winclaw.channel`、`winclaw.outcome`）
- `winclaw.message.duration_ms`（直方图，属性：`winclaw.channel`、`winclaw.outcome`）

队列 + 会话：

- `winclaw.queue.lane.enqueue`（计数器，属性：`winclaw.lane`）
- `winclaw.queue.lane.dequeue`（计数器，属性：`winclaw.lane`）
- `winclaw.queue.depth`（直方图，属性：`winclaw.lane` 或 `winclaw.channel=heartbeat`）
- `winclaw.queue.wait_ms`（直方图，属性：`winclaw.lane`）
- `winclaw.session.state`（计数器，属性：`winclaw.state`、`winclaw.reason`）
- `winclaw.session.stuck`（计数器，属性：`winclaw.state`）
- `winclaw.session.stuck_age_ms`（直方图，属性：`winclaw.state`）
- `winclaw.run.attempt`（计数器，属性：`winclaw.attempt`）
...
- `winclaw.webhook.error`
  - `winclaw.channel`、`winclaw.webhook`、`winclaw.chatId`、`winclaw.error`
- `winclaw.message.processed`
  - `winclaw.channel`、`winclaw.outcome`、`winclaw.chatId`、`winclaw.messageId`、`winclaw.sessionKey`、`winclaw.sessionId`、`winclaw.reason`
- `winclaw.session.stuck`
  - `winclaw.state`、`winclaw.ageMs`、`winclaw.queueDepth`、`winclaw.sessionKey`、`winclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `winclaw doctor`。
