...
- Replaces the legacy `gateway` command. CLI entry point: `winclaw gateway`.
...
winclaw gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.winclaw/winclaw.json` (or `WINCLAW_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__winclaw__/canvas/` from `~/.winclaw/workspace/canvas`. Disable with `canvasHost.enabled=false` or `WINCLAW_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `WINCLAW_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `WINCLAW_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.winclaw.*` may still exist)
- Linux: `winclaw-gateway-<profile>.service`
- Windows: `WinClaw Gateway (<profile>)`

Install metadata is embedded in the service config:

- `WINCLAW_SERVICE_MARKER=winclaw`
- `WINCLAW_SERVICE_KIND=gateway`
- `WINCLAW_SERVICE_VERSION=<version>`
...
```bash
winclaw --dev setup
winclaw --dev gateway --allow-unconfigured
...
- `WINCLAW_STATE_DIR=~/.winclaw-dev`
- `WINCLAW_CONFIG_PATH=~/.winclaw-dev/winclaw.json`
- `WINCLAW_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.winclaw/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `WINCLAW_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `WINCLAW_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `WINCLAW_CONFIG_PATH`
- unique `WINCLAW_STATE_DIR`
...
```bash
winclaw --profile main gateway install
winclaw --profile rescue gateway install
```

Example:

```bash
WINCLAW_CONFIG_PATH=~/.winclaw/a.json WINCLAW_STATE_DIR=~/.winclaw-a winclaw gateway --port 19001
WINCLAW_CONFIG_PATH=~/.winclaw/b.json WINCLAW_STATE_DIR=~/.winclaw-b winclaw gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `winclaw health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `winclaw`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `winclaw gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.winclaw.*` is cleaned up).
  - `winclaw doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are WinClaw profile services.
  ...
  - Cleanup: `winclaw gateway uninstall` (current service) and `winclaw doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `winclaw gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- WinClaw.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.winclaw.*` labels still unload cleanly).
- To stop it cleanly, use `winclaw gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `winclaw gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `winclaw gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

WinClaw installs a **systemd user service** by default on Linux/WSL2. We
...
`winclaw gateway install` writes the user unit. `winclaw doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/winclaw-gateway[-<profile>].service`:

```
[Unit]
Description=WinClaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/winclaw gateway --port 18789
Restart=always
RestartSec=5
Environment=WINCLAW_GATEWAY_TOKEN=
...
```
systemctl --user enable --now winclaw-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/winclaw-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now winclaw-gateway[-<profile>].service
```
...
- `winclaw gateway health|status` — request health/status over the Gateway WS.
- `winclaw message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `winclaw agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `winclaw gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `winclaw gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `winclaw gateway` and the legacy TCP control port.
