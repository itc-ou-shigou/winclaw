...
- During heartbeat runs, WinClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `winclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, WinClaw skips the heartbeat run entirely (no model call).
...
```bash
winclaw system event --text "Check for urgent follow-ups" --mode now
```
