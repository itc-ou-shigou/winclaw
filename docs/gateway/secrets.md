...
WinClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
When onboarding runs in interactive mode and you choose SecretRef storage, WinClaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). WinClaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/WinClaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/winclaw"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `winclaw.json` refs)
...
When reload-time activation fails after a healthy state, WinClaw enters degraded secrets state.
...
Credential-sensitive command paths that opt in (for example `winclaw memory` remote-memory paths and `winclaw qr --remote`) can resolve supported SecretRefs via gateway snapshot RPC.
...
- Snapshot refresh after backend secret rotation is handled by `winclaw secrets reload`.
...
```bash
winclaw secrets audit --check
winclaw secrets configure
winclaw secrets audit --check
```
...
- plaintext values at rest (`winclaw.json`, `auth-profiles.json`, `.env`)
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `winclaw.json` refs)
...
- lets you select supported secret-bearing fields in `winclaw.json` plus `auth-profiles.json` for one agent scope
...
- `winclaw secrets configure --providers-only`
- `winclaw secrets configure --skip-provider-setup`
- `winclaw secrets configure --agent <id>`
...
```bash
winclaw secrets apply --from /tmp/winclaw-secrets-plan.json
winclaw secrets apply --from /tmp/winclaw-secrets-plan.json --dry-run
```
...
WinClaw intentionally does not write rollback backups containing historical plaintext secret values.
