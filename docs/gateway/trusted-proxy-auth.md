...
read_when:
  - Running WinClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of WinClaw
...
- You run WinClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. WinClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. WinClaw extracts the user identity from the configured header
...
- WinClaw can stay on loopback HTTP behind the proxy.
...
If WinClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://winclaw.example.com
    to: http://winclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`winclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update WinClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `winclaw security audit` and review findings
