...
- Tighten local perms (`~/.winclaw` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).
...
4. **Re-run audit**
   - `winclaw security audit --deep` and confirm the report is clean.
...
authentication. WinClaw verifies the identity by resolving the
