...
```bash
winclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `winclaw tui --deliver`
...
- Local shell commands receive `WINCLAW_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `winclaw logs --follow`.
- Confirm the agent can run: `winclaw status` and `winclaw models status`.
...
- No agents in picker: check `winclaw agents list` and your routing config.
