...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/winclaw`)
...
If the page fails to load, start the Gateway first: `winclaw gateway`.
...
you revoke it with `winclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.winclaw/winclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop winclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. WinClaw
...
WinClaw **blocks** Control UI connections without device identity.
