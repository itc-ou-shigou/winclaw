---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for WinClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, WinClaw cron scheduling for periodic checks, or version status checks on a machine running WinClaw (laptop, workstation, Pi, VPS).
---

# WinClaw Host Hardening

## Overview

Assess and harden the host running WinClaw, then align it to a user-defined risk tolerance without breaking access. Use WinClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim WinClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. WinClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if WinClaw can access sensitive data.
...
### 2) Run WinClaw security audits (read-only)

As part of the default read-only checks, run `winclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `winclaw security audit` (faster, non-probing)
2. `winclaw security audit --json` (structured output)

Offer to apply WinClaw safe defaults (numbered):

1. `winclaw security audit --fix`

Be explicit that `--fix` only tightens WinClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check WinClaw version/update status (read-only)

As part of the default read-only checks, run `winclaw update status`.
...
- Credential hygiene notes (location of WinClaw creds, prefer disk encryption)
...
After WinClaw install or first hardening pass, run at least one baseline audit and version check:

- `winclaw security audit`
- `winclaw security audit --deep`
- `winclaw update status`

Ongoing monitoring is recommended. Use the WinClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `winclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `winclaw update status`
...
Before creating, `winclaw cron list` and match on exact `name`. If found, `winclaw cron edit <id> ...`.
If not found, `winclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `winclaw update status` (preferred for source checkouts and channels)
2. `npm view winclaw version` (published npm version)

## WinClaw command accuracy

Use only supported commands and flags:

- `winclaw security audit [--deep] [--fix] [--json]`
- `winclaw status` / `winclaw status --deep`
- `winclaw health --json`
- `winclaw update status`
- `winclaw cron add|list|runs|run`

Do not invent CLI flags or imply WinClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by WinClaw compaction:
