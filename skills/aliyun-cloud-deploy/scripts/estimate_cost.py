...
Output: Formatted cost table for user presentation.
"""
...
def format_markdown(rec):
    ...
    lines.append(f"")
    lines.append(f"| Service | Spec | Detail | Monthly (CNY) |")
    lines.append(f"|---------|------|--------|---------------|")
    for item in items:
        lines.append(
            f"| {item['service']} | {item['spec']} | {item['detail']} | {item['monthly_cost']:,} |"
        )
    lines.append(f"| **Total** | | | **~{total:,}** |")
    lines.append(f"")

    # Budget comparison
    budget = rec.get("requirements", {}).get("budget", 0)
    if budget > 0:
        diff = budget - total
        if diff >= 0:
            lines.append(f"> Budget: {budget:,} CNY/mo | Estimated: ~{total:,} CNY/mo | Surplus: {diff:,} CNY")
        else:
            lines.append(f"> **Warning**: Budget: {budget:,} CNY/mo | Estimated: ~{total:,} CNY/mo | Over by: {abs(diff):,} CNY")
...
def format_text_box(rec):
    ...
    cost_w = max(len(f"{i['monthly_cost']:,}") for i in items) if items else 6
    svc_w = max(svc_w, 7)  # "Service"
    spec_w = max(spec_w, 4)  # "Spec"
    cost_w = max(cost_w, len(f"{total:,}"), 6)
    ...
    lines.append(
        f"| {'Service':<{svc_w}} | {'Spec':<{spec_w}} | {'CNY/mo':>{cost_w}} |"
    )
    lines.append("+" + "-" * svc_w + "--+" + "-" * spec_w + "--+" + "-" * cost_w + "--+")
    for item in items:
        lines.append(
            f"| {item['service']:<{svc_w}} | {item['spec']:<{spec_w}} | {item['monthly_cost']:>{cost_w},} |"
        )
    lines.append("+" + "-" * svc_w + "--+" + "-" * spec_w + "--+" + "-" * cost_w + "--+")
    lines.append(
        f"| {'Total':<{svc_w}} | {'':<{spec_w}} | {'~' + f'{total:,}':>{cost_w}} |"
    )
    lines.append("+" + "=" * (total_w) + "+")
...
def main():
    parser = argparse.ArgumentParser(description="Format cost estimate")
