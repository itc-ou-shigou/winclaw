#!/usr/bin/env python3
"""
End-to-end test for azure-cloud-deploy skill scripts.

Tests all 6 patterns:
  1. analyze_requirements → recommendation JSON
  2. estimate_cost → formatted table
  3. generate_arm_template → ARM template JSON
...
TEST_CASES = [
    {
        "name": "lite",
        "args": "--budget 30 --traffic-now 50 --traffic-3m 200 --traffic-1y 400 --db none --security basic --app-type web --region eastus",
        "expect_pattern": "lite",
        "min_resources": 4,
    },
    {
        "name": "standard",
        "args": "--budget 100 --traffic-now 500 --traffic-3m 2000 --traffic-1y 4000 --db mysql --security standard --app-type web --region eastus",
        "expect_pattern": "standard",
        "min_resources": 2,
    },
    {
        "name": "ha",
        "args": "--budget 200 --traffic-now 3000 --traffic-3m 10000 --traffic-1y 40000 --db postgresql+redis --security standard --app-type web --region westeurope",
        "expect_pattern": "ha",
        "min_resources": 5,
    },
    {
        "name": "elastic",
        "args": "--budget 500 --traffic-now 10000 --traffic-3m 50000 --traffic-1y 200000 --db mysql+redis --security standard --app-type web --region eastus",
        "expect_pattern": "elastic",
        "min_resources": 6,
    },
    {
        "name": "serverless",
        "args": "--budget 100 --traffic-now 500 --traffic-3m 5000 --traffic-1y 20000 --db mysql --security basic --app-type api --region japaneast",
        "expect_pattern": "serverless",
        "min_resources": 4,
    },
    {
        "name": "container",
        "args": "--budget 400 --traffic-now 5000 --traffic-3m 20000 --traffic-1y 100000 --db mysql --security standard --app-type microservice --region eastus",
...
def test_pattern(tc):
    ...
    # Step 3: generate_arm_template
    with tempfile.NamedTemporaryFile(suffix=".json", delete=False, mode="w", encoding="utf-8") as tf:
        tf.write(stdout)
        rec_path = tf.name

    tpl_path = os.path.join(tempfile.gettempdir(), f"azure_{name}.json")
    cmd = f'python3 "{SCRIPTS_DIR}/generate_arm_template.py" --recommendation "{rec_path}" --output "{tpl_path}"'
    _, rc3 = run_cmd(cmd)
    if rc3 != 0:
        errors.append(f"generate_arm_template failed (rc={rc3})")
    else:
        try:
            with open(tpl_path, "r", encoding="utf-8") as f:
                tpl = json.load(f)
            num_res = len(tpl.get("resources", []))
            print(f"  ARM template: {num_res} resources")
            if num_res < tc["min_resources"]:
                errors.append(f"Too few resources: {num_res} < {tc['min_resources']}")
        except (json.JSONDecodeError, FileNotFoundError) as e:
            errors.append(f"ARM template read error: {e}")
    ...
    # Script format
    deploy_path = os.path.join(tempfile.gettempdir(), f"azure_deploy_{name}.sh")
    fmt = "script" if name in ("lite", "elastic") else "azcli" if name in ("standard", "ha") else "script"
    ...
    if name == "container":
        # Dockerfile
        df_path = os.path.join(tempfile.gettempdir(), f"azure_Dockerfile_{name}")
        ...
        # K8s manifest
        k8s_path = os.path.join(tempfile.gettempdir(), f"azure_k8s_{name}.yml")
    ...
    stats = {
        "cost": total_cost,
        "resources": num_res if 'num_res' in dir() else 0,
...
def main():
    print("=" * 60)
    print("Azure Cloud Deploy - End-to-End Test Suite")
    print("=" * 60)
