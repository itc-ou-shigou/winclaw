...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/winclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/winclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/winclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__WINCLAW_CONTROL_UI_BASE_PATH__ = "/winclaw";
    const app = mountApp("/winclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/winclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/winclaw/sessions");
  });
