...
This is an **alpha** iOS app that connects to an WinClaw Gateway as a `role: node`.
...
3. On iOS: WinClaw → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project WinClaw.xcodeproj -scheme WinClaw -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/WinClawKit` contains the shared transport/types used by the iOS app.
