...
final class MotionService: MotionServicing {
    func activities(params: WinClawMotionActivityParams) async throws -> WinClawMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[WinClawMotionActivityEntry], Error>) in
        ...
        return WinClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: WinClawPedometerParams) async throws -> WinClawPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<WinClawPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = WinClawPedometerPayload(
