...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: WinClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: WinClawProtocol.AnyCodable] = [
            "presence": .init([WinClawProtocol.AnyCodable(entry)]),
        ]
        let payload = WinClawProtocol.AnyCodable(payloadMap)
