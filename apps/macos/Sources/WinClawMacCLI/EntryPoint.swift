...
struct WinClawMacCLI {
    static func main() async {
        ...
        default:
            fputs("winclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      winclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      winclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      winclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      winclaw-mac connect
      winclaw-mac connect --url ws://127.0.0.1:18789 --json
      winclaw-mac discover --timeout 3000 --json
      winclaw-mac wizard --mode local
    """)
}
