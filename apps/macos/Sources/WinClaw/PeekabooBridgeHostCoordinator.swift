...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.winclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: WinClawPeekabooBridgeServices?
    private static var winclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("WinClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = WinClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.winclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.winclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class WinClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.winclaw.peekaboo")
