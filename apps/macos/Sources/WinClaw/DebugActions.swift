...
enum DebugActions {
    private static let verboseDefaultsKey = "winclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "winclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = WinClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "WinClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = WinClawPaths.configURL
        guard
