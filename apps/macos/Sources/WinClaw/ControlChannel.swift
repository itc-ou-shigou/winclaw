...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: WinClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.winclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: WinClawKit.AnyCodable]()) {
                $0[$1.key] = WinClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: WinClawProtocol.AnyCodable?) -> [String: WinClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: WinClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: WinClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: WinClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("winclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("winclaw.control.agent")
}
