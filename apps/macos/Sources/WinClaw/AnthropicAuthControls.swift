...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: WinClawOAuthStore.AnthropicOAuthStatus = WinClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: WinClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = WinClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = WinClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try WinClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. WinClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: WinClawOAuthStore.AnthropicOAuthStatus,
