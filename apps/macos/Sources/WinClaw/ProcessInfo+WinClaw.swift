...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.winclaw.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["WINCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "winclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "winclaw.nixMode") { return true }
