...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.winclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> WinClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: WinClawSessionPreviewEntry,
    ...
    private static func cache(payload: WinClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: WinClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [WinClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [WinClawKit.AnyCodable]) -> [WinClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(WinClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: WinClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: WinClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: WinClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: WinClawChatMessage) -> String? {
