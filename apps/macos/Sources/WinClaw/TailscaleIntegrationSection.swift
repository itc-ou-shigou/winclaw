...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.winclaw/winclaw.json. Prefer WINCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.winclaw/winclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.winclaw/winclaw.json. Restart the gateway to apply."
        }
