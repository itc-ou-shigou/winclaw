...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (WinClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: WinClawOAuthStore.AnthropicOAuthStatus = WinClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.winclaw", category: "anthropic-oauth")
...
enum WinClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let winclawOAuthDirEnv = "WINCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "WinClaw OAuth token file not found"
            case .unreadableFile: "WinClaw OAuth token file not readable"
            case .invalidJSON: "WinClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in WinClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "WinClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.winclawOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".winclaw", isDirectory: true)
