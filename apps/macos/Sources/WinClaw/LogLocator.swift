...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["WINCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/winclaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("winclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("winclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/winclaw/ (rolling or stdout), or nil if none exist.
