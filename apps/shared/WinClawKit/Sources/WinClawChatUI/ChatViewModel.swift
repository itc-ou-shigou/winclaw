...
private let chatUILogger = Logger(subsystem: "ai.winclaw", category: "WinClawChatUI")

@MainActor
@Observable
public final class WinClawChatViewModel {
    public private(set) var messages: [WinClawChatMessage] = []
    ...
    public var attachments: [WinClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [WinClawChatPendingToolCall] = []
    public private(set) var sessions: [WinClawChatSessionEntry] = []
    private let transport: any WinClawChatTransport
    ...
    private var pendingToolCallsById: [String: WinClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any WinClawChatTransport) {
    ...
    public var sessionChoices: [WinClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [WinClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: WinClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [WinClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: WinClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: WinClawChatMessage) -> WinClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> WinClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return WinClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: WinClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [WinClawChatMessage],
        incoming: [WinClawChatMessage]) -> [WinClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [WinClawChatMessage]) -> [WinClawChatMessage] {
        var result: [WinClawChatMessage] = []
    ...
    private static func dedupeKey(for message: WinClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [WinClawChatMessageContent] = [
            WinClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> WinClawChatAttachmentPayload in
            WinClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> WinClawChatSessionEntry {
        WinClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: WinClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: WinClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: WinClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = WinClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> WinClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
