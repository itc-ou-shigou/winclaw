...
@MainActor
struct WinClawChatComposer: View {
    @Bindable var viewModel: WinClawChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(WinClawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(WinClawChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \WinClawPendingAttachment.id)
                { (att: WinClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            WinClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(WinClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(WinClawChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(WinClawChatTheme.subtleCard)
        .clipShape(Capsule())
    }
