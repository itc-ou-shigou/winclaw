...
private final class TestChatTransport: @unchecked Sendable, WinClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [WinClawChatHistoryPayload]
    private let sessionsResponses: [WinClawChatSessionsListResponse]

    private let stream: AsyncStream<WinClawChatTransportEvent>
    private let continuation: AsyncStream<WinClawChatTransportEvent>.Continuation

    init(
        historyResponses: [WinClawChatHistoryPayload],
        sessionsResponses: [WinClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<WinClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> WinClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? WinClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [WinClawChatAttachmentPayload]) async throws -> WinClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return WinClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> WinClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? WinClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: WinClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = WinClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = WinClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { WinClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = WinClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = WinClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = WinClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { WinClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
