...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/winclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "WinClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/winclaw/scripts/mobile-reauth.sh'" --id winclaw-auth
        ;;
    *)
