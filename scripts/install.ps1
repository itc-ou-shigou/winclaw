# WinClaw Installer for Windows (PowerShell)
# Usage: iwr -useb https://winclaw.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://winclaw.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\winclaw",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 WinClaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one WinClaw.$NC" -Level info
    Write-Host ""
}
...
function Install-WinClawNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing WinClaw (winclaw@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g winclaw@$Version --no-fund --no-audit 2>&1
        Write-Host "WinClaw installed" -Level success
        return $true
    } catch {
...
function Install-WinClawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing WinClaw from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/winclaw/winclaw.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\winclaw\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\winclaw.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install WinClaw from git to $GitDir" -Level info
        } else {
            Install-WinClawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install WinClaw via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-WinClawNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'winclaw onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 WinClaw installed successfully!" -Level success
}
