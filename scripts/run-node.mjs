...
const shouldBuild = (deps) => {
  if (deps.env.WINCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.WINCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[winclaw] ${message}\n`);
};

const runWinClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["winclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
