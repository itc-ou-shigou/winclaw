...
INSTALL_URL="${WINCLAW_INSTALL_URL:-https://winclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${WINCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${WINCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="winclaw"
PACKAGE_NAME="${WINCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (WINCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${WINCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${WINCLAW_INSTALL_LATEST_OUT:-}"
fi
