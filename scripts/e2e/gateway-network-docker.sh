...
IMAGE_NAME="winclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="winclaw-net-e2e-$$"
GW_NAME="winclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "WINCLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "WINCLAW_SKIP_CHANNELS=1" \
	  -e "WINCLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "WINCLAW_SKIP_CRON=1" \
	  -e "WINCLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
