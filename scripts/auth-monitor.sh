...
#
# Suggested cron: */30 * * * * /home/admin/winclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send WinClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., winclaw-alerts)
...
STATE_FILE="$HOME/.winclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via WinClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use winclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via WinClaw to $NOTIFY_PHONE..."
            winclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! WinClaw is down. Run: ssh l36 '~/winclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
