#!/usr/bin/env bash
# ClawDock - Docker helpers for WinClaw
# Inspired by Simon Willison's "Running WinClaw in Docker"
# https://til.simonwillison.net/llms/winclaw-docker
#
# Installation:
...
# Common paths to check for WinClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/winclaw"
  "${HOME}/workspace/winclaw"
  "${HOME}/projects/winclaw"
  "${HOME}/dev/winclaw"
  "${HOME}/code/winclaw"
  "${HOME}/src/winclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found WinClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/winclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ WinClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/winclaw/winclaw.git ~/winclaw"
    echo "  cd ~/winclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^WINCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d winclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart winclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f winclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.winclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec winclaw-gateway \
    bash -c 'echo "alias winclaw=\"./winclaw.mjs\"" > /tmp/.bashrc_winclaw && bash --rcfile /tmp/.bashrc_winclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm winclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build winclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "WINCLAW_GATEWAY_TOKEN=$token" winclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" winclaw-gateway \
    bash -c './winclaw.mjs config set gateway.remote.token "$TOKEN" && ./winclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec winclaw-gateway \
    bash -c "./winclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart winclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm winclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec winclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'winclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for WinClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (winclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to winclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.winclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'winclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'winclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.winclaw.ai${_CLR_RESET}"
  echo ""
}
