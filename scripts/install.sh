...
# WinClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://winclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one WinClaw."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${WINCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 WinClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://winclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$WINCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.winclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_winclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.winclaw-* "$npm_root"/winclaw 2>/dev/null || true
}

extract_winclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_winclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/winclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/winclaw"|"/usr/local/bin/winclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/winclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale winclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing winclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing WinClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_winclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "WinClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*winclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_winclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_winclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "WinClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an winclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "WINCLAW_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "WINCLAW_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "WINCLAW_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "WINCLAW_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "WINCLAW_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "WINCLAW_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "WINCLAW_BETA" "CLAWDBOT_BETA"
map_legacy_env "WINCLAW_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "WINCLAW_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "WINCLAW_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "WINCLAW_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "WINCLAW_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "WINCLAW_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${WINCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${WINCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((WINCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${WINCLAW_NO_ONBOARD:-0}
NO_PROMPT=${WINCLAW_NO_PROMPT:-0}
DRY_RUN=${WINCLAW_DRY_RUN:-0}
INSTALL_METHOD=${WINCLAW_INSTALL_METHOD:-}
WINCLAW_VERSION=${WINCLAW_VERSION:-latest}
USE_BETA=${WINCLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/winclaw"
GIT_DIR=${WINCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${WINCLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${WINCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${WINCLAW_VERBOSE:-0}"
WINCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
WinClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://winclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/winclaw)
...
Environment variables:
  WINCLAW_INSTALL_METHOD=git|npm
  WINCLAW_VERSION=latest|next|<semver>
  WINCLAW_BETA=0|1
  WINCLAW_GIT_DIR=...
  WINCLAW_GIT_UPDATE=0|1
  WINCLAW_NO_PROMPT=1
  WINCLAW_DRY_RUN=1
  WINCLAW_NO_ONBOARD=1
  WINCLAW_VERBOSE=1
  WINCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://winclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected WinClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a WinClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_winclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"winclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://winclaw.ai/install.sh | bash"
}
...
ensure_winclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/winclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/winclaw" ]]; then
        ln -sf "$npm_root/winclaw/dist/entry.js" "${npm_bin}/winclaw"
        ui_info "Created winclaw bin link at ${npm_bin}/winclaw"
    fi
    return 0
}

# Check for existing WinClaw installation
check_existing_winclaw() {
    if [[ -n "$(type -P winclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing WinClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make winclaw show as \"command not found\" in new terminals."
...
warn_winclaw_not_found() {
    ui_warn "Installed, but winclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t winclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named winclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_winclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P winclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/winclaw" ]]; then
        echo "${npm_bin}/winclaw"
        return 0
    fi
...
install_winclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/winclaw/winclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing WinClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing WinClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning WinClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building WinClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/winclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/winclaw"
    ui_success "WinClaw wrapper installed to \$HOME/.local/bin/winclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view winclaw dist-tags.beta 2>/dev/null || true)"
...
install_winclaw() {
    local package_name="winclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            WINCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="winclaw"
        else
            WINCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${WINCLAW_VERSION}" ]]; then
        WINCLAW_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${WINCLAW_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing WinClaw v${resolved_version}"
    else
        ui_info "Installing WinClaw (${WINCLAW_VERSION})"
    fi
    local install_spec=""
    if [[ "${WINCLAW_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${WINCLAW_VERSION}"
    fi

    if ! install_winclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_winclaw_paths
        install_winclaw_npm "${install_spec}"
    fi

    if [[ "${WINCLAW_VERSION}" == "latest" && "${package_name}" == "winclaw" ]]; then
        if ! resolve_winclaw_bin &> /dev/null; then
            ui_warn "npm install winclaw@latest failed; retrying winclaw@next"
            cleanup_npm_winclaw_paths
            install_winclaw_npm "winclaw@next"
        fi
    fi

    ensure_winclaw_bin_link || true

    ui_success "WinClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${WINCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_winclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (winclaw not on PATH yet)"
        warn_winclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${WINCLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.winclaw/workspace-${profile}"
    else
        echo "${HOME}/.winclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${WINCLAW_CONFIG_PATH:-$HOME/.winclaw/winclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run winclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but winclaw not on PATH; skipping onboarding"
        warn_winclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run winclaw onboard to retry"
        return
    }
}

resolve_winclaw_version() {
    local version=""
    local claw="${WINCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v winclaw &> /dev/null; then
        claw="$(command -v winclaw)"
    fi
    if [[ -n "$claw" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/winclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/winclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_winclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found WinClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set WINCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_winclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing WinClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g winclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g winclaw 2>/dev/null || true
        ...
        install_winclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/winclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/winclaw"
    ...
    WINCLAW_BIN="$(resolve_winclaw_bin || true)"
    ...
    installed_version=$(resolve_winclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 WinClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 WinClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/winclaw"
        ui_kv "Update command" "winclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://winclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running winclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                WINCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                WINCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                WINCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run winclaw doctor and winclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run winclaw onboard later"
        else
            local config_path="${WINCLAW_CONFIG_PATH:-$HOME/.winclaw/winclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (winclaw not on PATH yet)"
                    warn_winclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run winclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v winclaw &> /dev/null; then
        local claw="${WINCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_winclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (winclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if WINCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: winclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${WINCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
