#!/bin/bash
# Setup WinClaw Auth Management System
...
echo "=== WinClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'winclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  WinClaw message: Send warning via WinClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/winclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/winclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/winclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now winclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status winclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
