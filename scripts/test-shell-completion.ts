 ...
 * `winclaw update`. Use it to verify the completion installation flow
...
\`winclaw update\`, \`winclaw doctor\`, and \`winclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# WinClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`winclaw completion --install\` later to enable.`));
    return;
  }
