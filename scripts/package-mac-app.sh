...
# Build and bundle WinClaw into a minimal .app we can open.
# Outputs to dist/WinClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/WinClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="WinClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.winclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/winclaw/winclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/WinClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :WinClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :WinClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/WinClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/WinClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/WinClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/WinClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/WinClaw/Resources/WinClaw.icns" "$APP_ROOT/Contents/Resources/WinClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/WinClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying WinClawKit resources"
WINCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/WinClawKit_WinClawKit.bundle"
if [ -d "$WINCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/WinClawKit_WinClawKit.bundle"
  cp -R "$WINCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/WinClawKit_WinClawKit.bundle"
else
  echo "WARN: WinClawKit resource bundle not found at $WINCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running WinClaw"
killall -q WinClaw 2>/dev/null || true
