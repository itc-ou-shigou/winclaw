...
1. [SourceForge](https://sourceforge.net/projects/winclaw/files/WinClawSetup-2026.3.1.exe/download) または [GitHub Releases](https://github.com/itc-ou-shigou/winclaw/releases) から `WinClawSetup-{version}.exe` をダウンロード（リポジトリの [`releases/`](releases/) ディレクトリからも取得可能）
...
WinClaw には 3 つのクラウドデプロイスキルが内蔵されており、要件ヒアリングからインフラ構築、コードデプロイまでを WinClaw Chat での自然な会話で完了できます。
...
Phase 3C: 検証            → ヘルスチェック実行、アクセス URL 付きデプロイレポート生成
```
...
2. **質問に回答** — WinClaw がプロジェクトを検出し、予算・トラフィック・DB の要件を質問
3. **設計レビュー** — 推奨アーキテクチャとコスト内訳を確認。承認または調整
4. **デプロイ** — WinClaw がインフラテンプレートを生成し、リソースをプロビジョニングし、コードをデプロイ
5. **検証** — ヘルスチェックを実行し、アクセス URL 付きのデプロイレポートを提供
