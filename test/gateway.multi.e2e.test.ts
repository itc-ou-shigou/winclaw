...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `winclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".winclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          WINCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        WINCLAW_GATEWAY_TOKEN: inst.gatewayToken,
        WINCLAW_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          WINCLAW_GATEWAY_PORT: String(gwA.port),
          WINCLAW_GATEWAY_TOKEN: gwA.gatewayToken,
          WINCLAW_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          WINCLAW_GATEWAY_PORT: String(gwB.port),
          WINCLAW_GATEWAY_TOKEN: gwB.gatewayToken,
          WINCLAW_GATEWAY_PASSWORD: "",
        }),
