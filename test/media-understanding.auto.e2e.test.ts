...
import type { WinClawConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("winclaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("winclaw-bin-sherpa-");
      const modelDir = await makeTempDir("winclaw-sherpa-model-");
      ...
      const cfg: WinClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("winclaw-bin-whispercpp-");
      const modelDir = await makeTempDir("winclaw-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("winclaw-bin-gemini-");
      ...
      const cfg: WinClawConfig = { tools: { media: { image: {} } } };
