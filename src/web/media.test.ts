...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredWinClawTmpDir(), "winclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub WINCLAW_STATE_DIR.
    // Also keep it outside the WinClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["WINCLAW_STATE_DIR"]);
    process.env.WINCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "winclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows default WinClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default WinClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
