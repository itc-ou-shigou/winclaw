...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        WINCLAW_BASE_ENV: "base",
        WINCLAW_TO_REMOVE: undefined,
      },
      env: {
        WINCLAW_TEST_ENV: "ok",
      },
    ...
    expect(resolved.WINCLAW_BASE_ENV).toBe("base");
    expect(resolved.WINCLAW_TEST_ENV).toBe("ok");
    expect(resolved.WINCLAW_TO_REMOVE).toBeUndefined();
  });
