...
describe("shared/frontmatter", () => {
  ...
  test("resolveWinClawManifestBlock parses JSON5 metadata and picks winclaw block", () => {
    const frontmatter = {
      metadata: "{ winclaw: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveWinClawManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveWinClawManifestBlock returns undefined for invalid input", () => {
    expect(resolveWinClawManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveWinClawManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveWinClawManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
