...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "winclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".winclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<WinClawConfig> = {},
): WinClawConfig {
  const base: WinClawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<WinClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<WinClawConfig>,
