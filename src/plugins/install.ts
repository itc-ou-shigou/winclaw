...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing winclaw.extensions; update the plugin package to include winclaw.extensions (for example ["./dist/index.js"]). See https://docs.winclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-winclaw-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_WINCLAW_EXTENSIONS: "missing_winclaw_extensions",
  EMPTY_WINCLAW_EXTENSIONS: "empty_winclaw_extensions",
...
function ensureWinClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_WINCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json winclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_WINCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureWinClawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from winclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-winclaw"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "winclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "winclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "winclaw-npm-pack-",
    spec,
    timeoutMs,
