...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `winclaw-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "winclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "winclaw.plugin.json");
  const linkedManifest = path.join(rootDir, "winclaw.plugin.json");
