...
describe("canvas host state dir defaults", () => {
  ...
  it("uses WINCLAW_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("WinClaw Canvas");
    } finally {
      await handler.close();
