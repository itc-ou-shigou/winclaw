...
function makeConfig(): WinClawConfig {
  return {
    secrets: {
      providers: {
        default: { source: "env" },
...
describe("resolveOnboardingSecretInputString", () => {
  it("resolves env-template SecretInput strings", async () => {
    const resolved = await resolveOnboardingSecretInputString({
      config: makeConfig(),
      value: "${WINCLAW_GATEWAY_PASSWORD}",
      path: "gateway.auth.password",
      env: {
        WINCLAW_GATEWAY_PASSWORD: "gateway-secret",
      },
    });
  ...
  it("throws with path context when env-template SecretRef cannot resolve", async () => {
    await expect(
      ...
      'gateway.auth.password: failed to resolve SecretRef "env:default:WINCLAW_GATEWAY_PASSWORD"',
    );
  });
});
