  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ...
        ["winclaw update --dry-run", "Preview actions without changing anything"],
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.winclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.winclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
