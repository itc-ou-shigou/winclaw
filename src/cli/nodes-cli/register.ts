    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["winclaw nodes status", "List known nodes with live status."],
          ["winclaw nodes pairing pending", "Show pending node pairing requests."],
          ['winclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["winclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.winclaw.ai/cli/nodes")}\n`,
    );
