...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "winclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "winclaw-bundled-skills-test-"));
    process.env.WINCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
