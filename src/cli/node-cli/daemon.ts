...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("winclaw node install"), formatCliCommand("winclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("winclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    WINCLAW_LOG_PREFIX: baseEnv.WINCLAW_LOG_PREFIX ?? "node",
