...
describe("update-cli", () => {
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-update-"));
    try {
      await fs.writeFile(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-update-wizard-"));
    const previousGitDir = process.env.WINCLAW_GIT_DIR;
    try {
      setTty(true);
      process.env.WINCLAW_GIT_DIR = tempDir;
      ...
      process.env.WINCLAW_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
