...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.WINCLAW_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.WINCLAW_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<WinClawConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live WinClaw docs",
    hasSubcommands: false,
    register: async (program) => {
