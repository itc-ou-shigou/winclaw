    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["winclaw memory status", "Show index and provider status."],
          ["winclaw memory index --force", "Force a full reindex."],
          ['winclaw memory search --query "deployment notes"', "Search indexed memory entries."],
          ["winclaw memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.winclaw.ai/cli/memory")}\n`,
    );
