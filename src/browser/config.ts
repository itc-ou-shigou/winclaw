import type { BrowserConfig, BrowserProfileConfig, WinClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_WINCLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_WINCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "winclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_WINCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_WINCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an WinClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for winclaw/winclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: WinClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_WINCLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_WINCLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_WINCLAW_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "winclaw";
