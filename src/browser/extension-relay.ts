...
const RELAY_AUTH_HEADER = "x-winclaw-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/WinClaw-Extension-Relay",
            revision: "0",
            userAgent: "WinClaw-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "WinClaw/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedWinClawRelay({
