...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".winclaw-profile-decorated");
}
 ...
 */
export function decorateWinClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_WINCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_WINCLAW_BROWSER_COLOR).toUpperCase();
