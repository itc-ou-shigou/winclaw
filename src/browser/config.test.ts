...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("winclaw");
    expect(profile?.driver).toBe("winclaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const winclaw = resolveProfile(resolved, "winclaw");
    expect(winclaw?.driver).toBe("winclaw");
    expect(winclaw?.cdpPort).toBe(18800);
    expect(winclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from WINCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ WINCLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const winclaw = resolveProfile(resolved, "winclaw");
      expect(winclaw?.cdpPort).toBe(19012);
      expect(winclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ WINCLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const winclaw = resolveProfile(resolved, "winclaw");
      expect(winclaw?.cdpPort).toBe(19022);
      expect(winclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(winclaw?.cdpPort).toBe(19000);
    expect(winclaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "winclaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        winclaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("winclaw");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to winclaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps winclaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("winclaw");
    ...
    it("keeps winclaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("winclaw");
    ...
    it("keeps winclaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
