...
function localWinClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "winclaw",
...
function createLocalWinClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localWinClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveWinClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "winclaw-reset-"));
    const profileDir = path.join(tempRoot, "winclaw");
    ...
    const ops = createLocalWinClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveWinClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "winclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "winclaw");
    fs.mkdirSync(profileDir, { recursive: true });
