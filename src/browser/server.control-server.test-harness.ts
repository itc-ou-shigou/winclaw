...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/winclaw" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          winclaw: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.WINCLAW_GATEWAY_PORT;
  process.env.WINCLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.WINCLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.WINCLAW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.WINCLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.WINCLAW_GATEWAY_PASSWORD;
  } else {
    process.env.WINCLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
