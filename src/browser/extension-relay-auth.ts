...
const RELAY_TOKEN_CONTEXT = "winclaw-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const WINCLAW_RELAY_BROWSER = "WinClaw/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.WINCLAW_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or WINCLAW_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedWinClawRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === WINCLAW_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
