...
let originalWinClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.WINCLAW_TEST_FILE_LOG;
  originalWinClawLogLevel = process.env.WINCLAW_LOG_LEVEL;
  delete process.env.WINCLAW_TEST_FILE_LOG;
  delete process.env.WINCLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.WINCLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalWinClawLogLevel === undefined) {
    delete process.env.WINCLAW_LOG_LEVEL;
  } else {
    process.env.WINCLAW_LOG_LEVEL = originalWinClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
