...
function cfg(accounts?: Record<string, unknown> | null): WinClawConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as WinClawConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as WinClawConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as WinClawConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as WinClawConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as WinClawConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as WinClawConfig)).toBe("default");
    });
  });
});
