...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in WinClaw config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "WINCLAW_SECRET",
      envVarFormatError:
        'Use an env var name like "WINCLAW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
