...
function setSlackDmPolicy(cfg: WinClawConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "WinClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for WinClaw`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: WinClawConfig, allowFrom: string[]): WinClawConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
