...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: WinClawConfig, policy: ChannelAccessPolicy) => WinClawConfig;
  resolveAllowlist: (params: { cfg: WinClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: WinClawConfig; resolved: TResolved }) => WinClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: WinClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: WinClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: WinClawConfig, policy: ChannelAccessPolicy): WinClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: WinClawConfig, policy: ChannelAccessPolicy): WinClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: WinClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: WinClawConfig; resolved: string[] }) => {
      calls.push("apply");
