...
const execHostEnforced = process.env.WINCLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.WINCLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureWinClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
