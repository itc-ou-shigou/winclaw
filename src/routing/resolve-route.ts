...
function listAgents(cfg: WinClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: WinClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<WinClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: WinClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: WinClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: WinClawConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
  byChannelAccountIndex: Map<string, EvaluatedBindingsIndex>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<WinClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  WinClawConfig,
  {
    bindingsRef: WinClawConfig["bindings"];
    agentsRef: WinClawConfig["agents"];
    sessionRef: WinClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: WinClawConfig): Map<string, ResolvedAgentRoute> {
