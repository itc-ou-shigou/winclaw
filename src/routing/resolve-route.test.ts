...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: WinClawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: WinClawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/winclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<WinClawConfig["bindings"]>[number];
