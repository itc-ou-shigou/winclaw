...
describe("resolveGatewayServiceDescription", () => {
  it("prefers explicit description override", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { WINCLAW_PROFILE: "work", WINCLAW_SERVICE_VERSION: "1.0.0" },
        description: "Custom",
      }),
    ).toBe("Custom");
  });
  ...
  it("resolves version from explicit environment map", () => {
    expect(
      resolveGatewayServiceDescription({
        env: { WINCLAW_PROFILE: "work", WINCLAW_SERVICE_VERSION: "local" },
        environment: { WINCLAW_SERVICE_VERSION: "remote" },
      }),
    ).toBe("WinClaw Gateway (profile: work, vremote)");
  });
});
