...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.WINCLAW_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service WINCLAW_GATEWAY_TOKEN does not match gateway.auth.token in winclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `winclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
