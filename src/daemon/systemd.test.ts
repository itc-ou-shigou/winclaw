...
describe("isSystemdServiceEnabled", () => {
  ...
  it("calls systemctl is-enabled when systemctl is present", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "winclaw-gateway.service"]);
      cb(null, "enabled", "");
    });
    const result = await isSystemdServiceEnabled({ env: {} });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "winclaw-gateway.service"]);
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled winclaw-gateway.service",
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when WINCLAW_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/winclaw-gateway.service",
    },
    {
      name: "uses profile-specific service name when WINCLAW_PROFILE is set to a custom value",
      env: { HOME: "/home/test", WINCLAW_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/winclaw-gateway-jbphoenix.service",
    },
    {
      name: "prefers WINCLAW_SYSTEMD_UNIT over WINCLAW_PROFILE",
      env: {
        HOME: "/home/test",
    ...
    {
      name: "handles WINCLAW_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
    ...
    {
      name: "trims whitespace from WINCLAW_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        WINCLAW_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/winclaw gateway start --name "My Bot"')).toEqual([
      "/usr/bin/winclaw",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('winclaw --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["winclaw", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('winclaw --path "C:\\\\Program Files\\\\WinClaw"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["winclaw", "--path", "C:\\\\Program Files\\\\WinClaw"]);

    expect(
      splitArgsPreservingQuotes('winclaw --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["winclaw", "--label", 'My "Quoted" Name']);
  });
});
...
describe("systemd service control", () => {
  ...
  it("stops the resolved user unit", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "stop", "winclaw-gateway.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "winclaw-gateway-work.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
    const stdout = { write } as unknown as NodeJS.WritableStream;

    await restartSystemdService({ stdout, env: { WINCLAW_PROFILE: "work" } });
  ...
  it("keeps direct --user scope when SUDO_USER is root", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "winclaw-gateway.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
    const stdout = { write } as unknown as NodeJS.WritableStream;
