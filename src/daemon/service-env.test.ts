...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.WINCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.WINCLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.WINCLAW_SERVICE_MARKER).toBe("winclaw");
    expect(env.WINCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.WINCLAW_SERVICE_VERSION).toBe("string");
    expect(env.WINCLAW_SYSTEMD_UNIT).toBe("winclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.WINCLAW_LAUNCHD_LABEL).toBe("ai.winclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", WINCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.WINCLAW_SYSTEMD_UNIT).toBe("winclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.WINCLAW_LAUNCHD_LABEL).toBe("ai.winclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through WINCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", WINCLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.WINCLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to WINCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.WINCLAW_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers WINCLAW_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        WINCLAW_GATEWAY_TOKEN: "winclaw-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.WINCLAW_GATEWAY_TOKEN).toBe("winclaw-token");
  });

  it("omits WINCLAW_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.WINCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
