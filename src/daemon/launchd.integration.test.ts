...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `winclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      WINCLAW_LAUNCHD_LABEL: `ai.winclaw.launchd-int-${testId}`,
      WINCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
