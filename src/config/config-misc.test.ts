...
import { buildWebSearchProviderConfig, withTempHome, writeWinClawConfig } from "./test-helpers.js";
import { WinClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = WinClawSchema.safeParse({
      $schema: "https://winclaw.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://winclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = WinClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = WinClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeWinClawConfig(home, {
        gateway: { bind: "${WINCLAW_BIND}" },
      });

      const prev = process.env.WINCLAW_BIND;
      process.env.WINCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.WINCLAW_BIND;
        } else {
          process.env.WINCLAW_BIND = prev;
        }
      }
    });
  });
