...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("winclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "winclaw-sandbox-browser-global" },
      agentBrowser: { network: "winclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("winclaw-sandbox-browser-agent");
  });
