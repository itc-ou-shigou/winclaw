...
import type { ConfigFileSnapshot } from "./types.winclaw.js";
import { WinClawSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(WinClawSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.winclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/winclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/winclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/winclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/winclaw/nickserv-password.txt");
