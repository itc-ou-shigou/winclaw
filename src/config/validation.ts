...
import type { WinClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: WinClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: WinClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: WinClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = WinClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as WinClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as WinClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as WinClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as WinClawConfig,
  };
}
