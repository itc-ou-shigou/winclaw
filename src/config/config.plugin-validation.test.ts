...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "winclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    WINCLAW_PLUGIN_MANIFEST_CACHE_MS: process.env.WINCLAW_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-config-plugin-validation-"));
    ...
    process.env.WINCLAW_STATE_DIR = path.join(suiteHome, ".winclaw");
    process.env.WINCLAW_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.WINCLAW_STATE_DIR === undefined) {
      delete process.env.WINCLAW_STATE_DIR;
    } else {
      process.env.WINCLAW_STATE_DIR = envSnapshot.WINCLAW_STATE_DIR;
    }
    if (envSnapshot.WINCLAW_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.WINCLAW_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.WINCLAW_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.WINCLAW_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
