...
export function isCommandEnabled(cfg: WinClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: WinClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as WinClawConfig),
...
export function getCommandDetection(_cfg?: WinClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: WinClawConfig) {
