...
function createDiscordParams(commandBody: string, cfg: WinClawConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: WinClawConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

async function runDiscordAcpCommand(commandBody: string, cfg: WinClawConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: WinClawConfig = baseCfg) {
