...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "winclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "winclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.WINCLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.WINCLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.WINCLAW_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[winclaw] ${parsed.error}`);
      process.exit(2);
    }
