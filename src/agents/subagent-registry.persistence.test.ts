...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["WINCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-subagent-"));
  ...
  it("uses isolated temp state when WINCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "winclaw-test-state"));
  });
});
