...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "winclaw.sandbox=1");
  args.push("--label", `winclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `winclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `winclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "winclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`winclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`winclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("winclaw sandbox recreate --all");
}
