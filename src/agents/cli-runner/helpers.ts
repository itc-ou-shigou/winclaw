...
/**
 * Cleanup suspended WinClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: WinClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-cli-images-"));
  const paths: string[] = [];
