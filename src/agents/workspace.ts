...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.WINCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".winclaw", `workspace-${profile}`);
  }
  return path.join(home, ".winclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".winclaw";
