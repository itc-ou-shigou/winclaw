...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    winclaw: process.env.WINCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.winclaw === undefined) {
      delete process.env.WINCLAW_GATEWAY_TOKEN;
    } else {
      process.env.WINCLAW_GATEWAY_TOKEN = envSnapshot.winclaw;
    }
  ...
  it("uses WINCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.WINCLAW_GATEWAY_TOKEN = "local-env-token";
