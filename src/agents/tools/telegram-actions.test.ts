...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): WinClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as WinClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): WinClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as WinClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: WinClawConfig, accountId = "media") {
    await handleTelegramAction(
