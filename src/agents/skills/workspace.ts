...
function resolveSkillsLimits(config?: WinClawConfig): ResolvedSkillsLimits {
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: WinClawConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`winclaw skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
