...
import {
  ...
  normalizeStringList,
  parseWinClawManifestInstallBase,
  parseFrontmatterBool,
  resolveWinClawManifestBlock,
  resolveWinClawManifestInstall,
  resolveWinClawManifestOs,
  resolveWinClawManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  WinClawSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseWinClawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveWinClawMetadata(
  frontmatter: ParsedSkillFrontmatter,
): WinClawSkillMetadata | undefined {
  const metadataObj = resolveWinClawManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveWinClawManifestRequires(metadataObj);
  const install = resolveWinClawManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveWinClawManifestOs(metadataObj);
  return {
