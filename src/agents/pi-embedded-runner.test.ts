...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-embedded-agent-"));
...
const ensureModels = (cfg: WinClawConfig) => ensureWinClawModelsJson(cfg, agentDir) as unknown;
