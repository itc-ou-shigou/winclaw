 ...
 * A common mistake — caused by the similarity with the `winclaw.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (winclaw.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.WINCLAW_AUTH_STORE_READONLY === "1";
