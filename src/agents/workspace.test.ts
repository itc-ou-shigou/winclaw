...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses WINCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      WINCLAW_HOME: "/srv/winclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/winclaw-home"), ".winclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".winclaw", "workspace-state.json"] as const;
