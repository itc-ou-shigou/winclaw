...
const originalBundledDir = process.env.WINCLAW_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("winclaw-bundled-");
  const workspaceDir = await createTempDir("winclaw-workspace-");
  const pluginRoot = path.join(bundledPluginsDir, "diffs");

  await fs.mkdir(path.join(pluginRoot, "skills", "diffs"), { recursive: true });
  await fs.writeFile(
    path.join(pluginRoot, "winclaw.plugin.json"),
    JSON.stringify(
      {
        id: "diffs",
        skills: ["./skills"],
...
afterEach(async () => {
  process.env.WINCLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.WINCLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
