...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.WINCLAW_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-test-"));
    process.env.WINCLAW_STATE_DIR = stateDir;
    process.env.WINCLAW_PROFILE = "isolated";

    try {
      const tool = createWinClawTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: winclaw --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/winclaw" } }\n}\n';
