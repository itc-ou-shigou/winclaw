...
function buildCoreToolGroupMap() {
  ...
  const winclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInWinClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:winclaw": winclawTools,
