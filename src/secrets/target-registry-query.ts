...
const WINCLAW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of WINCLAW_COMPILED_SECRET_TARGETS) {
...
const WINCLAW_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries =
    allowedTargetIds === null
      ? WINCLAW_COMPILED_SECRET_TARGETS
      : Array.from(allowedTargetIds).flatMap(
          (targetId) => WINCLAW_TARGETS_BY_ID.get(targetId) ?? [],
        );
