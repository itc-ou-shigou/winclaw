...
function buildFixturePaths(rootDir: string) {
  const stateDir = path.join(rootDir, ".winclaw");
  return {
    rootDir,
    stateDir,
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      WINCLAW_STATE_DIR: paths.stateDir,
      WINCLAW_CONFIG_PATH: paths.configPath,
