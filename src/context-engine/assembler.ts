import type { ContextEngine } from "winclaw/plugin-sdk";
...
/**
 * Detect if a raw block is a WinClaw-normalised OpenAI reasoning item.
 * WinClaw converts OpenAI `{type:"reasoning", id:"rs_…", encrypted_content:"…"}`
...
export function toolCallBlockFromPart(part: MessagePartRecord, rawType?: string): unknown {
  ...
  if (input !== undefined) {
    ...
    // Completions extractToolCalls and Responses API paths in WinClaw).
...
export function blockFromPart(part: MessagePartRecord): unknown {
  ...
  if (metadata.raw && typeof metadata.raw === "object") {
    // If this is a WinClaw-normalised OpenAI reasoning block, restore the original
