...
function resolveAuth(cfg: WinClawConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.WINCLAW_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayPasswordSecretRef(
  ...
  const hasPasswordEnvCandidate = Boolean(
    env.WINCLAW_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim(),
  );
  ...
  if (mode !== "password") {
    const hasTokenCandidate =
      Boolean(env.WINCLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim()) ||
