...
homepage: https://docs.winclaw.ai/automation/hooks#command-logger
metadata:
  {
    "winclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.winclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.winclaw/logs/commands.log ~/.winclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/winclaw`:
   ```
   /home/username/.winclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
