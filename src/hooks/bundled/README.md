...
This directory contains hooks that ship with WinClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.winclaw/winclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'winclaw.*gateway' && pnpm winclaw gateway`
3. Enable the hook: `winclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.winclaw.ai/automation/hooks
