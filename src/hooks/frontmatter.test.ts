...
`;
    ...
    expect(parsed.winclaw.emoji).toBe("💾");
    expect(parsed.winclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.winclaw.emoji).toBe("📝");
    expect(parsed.winclaw.events).toEqual(["command"]);
    expect(parsed.winclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.winclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"winclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"winclaw": {"events": ["test"]}}');
  });
...
describe("resolveWinClawMetadata", () => {
  it("extracts winclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveWinClawMetadata(frontmatter);
  ...
  it("returns undefined when winclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        winclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with WinClaw" },
            { id: "npm", kind: "npm", package: "@winclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@winclaw/hook");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const winclaw = resolveWinClawMetadata(frontmatter);
    expect(winclaw?.emoji).toBe("disk");
    expect(winclaw?.events).toEqual(["command:new"]);
  });
});
