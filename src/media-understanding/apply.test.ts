...
const TEMP_MEDIA_PREFIX = "winclaw-media-";
...
function createGroqAudioConfig(): WinClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): WinClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): WinClawConfig {
  return {
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredWinClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        WINCLAW_AGENT_DIR: isolatedAgentDir,
