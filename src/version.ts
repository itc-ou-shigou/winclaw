...
const CORE_PACKAGE_NAME = "winclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["WINCLAW_VERSION"],
      runtimeVersion,
      env["WINCLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current WinClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __WINCLAW_VERSION__ === "string" ? __WINCLAW_VERSION__ : undefined,
  bundledVersion: process.env.WINCLAW_BUNDLED_VERSION,
});
