...
import { coreGatewayHandlers, initGrcConnectionGetter } from "./server-methods.js";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.WINCLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.WINCLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "WINCLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "WINCLAW_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("winclaw doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("winclaw doctor")}" to repair, then retry.`,
    );
  }
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `winclaw config set gateway.auth.mode token` and `winclaw config set gateway.auth.token <token>`.",
      );
    }
  }
  cfgAtStart = (
  ...
  if (!minimalTestGateway) {
    ({ browserControl, pluginServices, grcConnection } = await startGatewaySidecars({
      cfg: cfgAtStart,
      pluginRegistry,
      defaultWorkspaceDir,
      deps,
  ...
  // Wire the live GrcConnectionManager into the pre-built GRC handler table so
  // that grc.status, grc.sync, grc.pair, and grc.pairVerify can use it.
  initGrcConnectionGetter(() => grcConnection);
