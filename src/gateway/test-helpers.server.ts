...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "WINCLAW_STATE_DIR",
  "WINCLAW_CONFIG_PATH",
  "WINCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "WINCLAW_SKIP_GMAIL_WATCHER",
  "WINCLAW_SKIP_CANVAS_HOST",
  "WINCLAW_BUNDLED_PLUGINS_DIR",
  "WINCLAW_SKIP_CHANNELS",
  "WINCLAW_SKIP_PROVIDERS",
  "WINCLAW_SKIP_CRON",
  "WINCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
function applyGatewaySkipEnv() {
  ...
  process.env.WINCLAW_SKIP_PROVIDERS = "1";
  process.env.WINCLAW_SKIP_CRON = "1";
  process.env.WINCLAW_TEST_MINIMAL_GATEWAY = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "winclaw-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".winclaw-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__winclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__winclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["WINCLAW_GATEWAY_TOKEN"]);
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.WINCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
