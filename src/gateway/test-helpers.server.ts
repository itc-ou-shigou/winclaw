...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.WINCLAW_STATE_DIR;
  previousConfigPath = process.env.WINCLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.WINCLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.WINCLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.WINCLAW_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.WINCLAW_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.WINCLAW_STATE_DIR = path.join(tempHome, ".winclaw");
  delete process.env.WINCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.WINCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.WINCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.WINCLAW_SKIP_CANVAS_HOST = "1";
  process.env.WINCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "winclaw-test-no-bundled-extensions")
    : "winclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "winclaw-test-"))
    : path.join(tempHome, ".winclaw-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.WINCLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.WINCLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.WINCLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.WINCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.WINCLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.WINCLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.WINCLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.WINCLAW_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.WINCLAW_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.WINCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.WINCLAW_GATEWAY_TOKEN = fallbackToken;
  }
