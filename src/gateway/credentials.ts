...
function throwUnresolvedGatewaySecretInput(path: string): never {
  throw new Error(
    [
      ...
      "Fix: set WINCLAW_GATEWAY_TOKEN/WINCLAW_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.WINCLAW_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.WINCLAW_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
