...
let lastCreateWinClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/winclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateWinClawToolsContext?.agentTo,
          agentThreadId: lastCreateWinClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createWinClawTools: (ctx: Record<string, unknown>) => {
      lastCreateWinClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateWinClawToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateWinClawToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-winclaw-message-to": "channel:24514",
        "x-winclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
