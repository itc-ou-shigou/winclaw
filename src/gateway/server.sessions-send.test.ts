...
const { createWinClawTools } = await import("../agents/winclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createWinClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createWinClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["WINCLAW_GATEWAY_PORT", "WINCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.WINCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.WINCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("WINCLAW_CONFIG_PATH missing in gateway test environment");
      }
