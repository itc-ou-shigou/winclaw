...
describe("resolveGatewayConnection", () => {
  ...
  it("uses WINCLAW_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ WINCLAW_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers WINCLAW_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ WINCLAW_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
