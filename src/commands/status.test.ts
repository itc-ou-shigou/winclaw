...
beforeAll(() => {
  envSnapshot = captureEnv(["WINCLAW_PROFILE"]);
  process.env.WINCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/winclaw-root.js", () => ({
  resolveWinClawPackageRoot: vi.fn().mockResolvedValue("/tmp/winclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/winclaw/pnpm-lock.yaml",
      markerPath: "/tmp/winclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.winclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.winclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("WinClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("winclaw status --all") ||
          l.includes("winclaw --profile isolated status --all") ||
          l.includes("winclaw status --all") ||
          l.includes("winclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.WINCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
