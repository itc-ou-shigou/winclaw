...
function pruneAuthProfiles(
  cfg: WinClawConfig,
  profileIds: Set<string>,
): { next: WinClawConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("winclaw models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("winclaw models auth setup-token")} or ${formatCliCommand(
      "winclaw configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "winclaw models auth login --provider openai-codex",
    )} or ${formatCliCommand("winclaw configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("winclaw configure")}\` or \`${formatCliCommand("winclaw onboard")}\`.`;
}
