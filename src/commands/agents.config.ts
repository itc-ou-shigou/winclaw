...
function resolveAgentName(cfg: WinClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: WinClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: WinClawConfig): AgentSummary[] {
