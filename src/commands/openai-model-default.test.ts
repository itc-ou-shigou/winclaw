...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: WinClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: WinClawConfig) => { changed: boolean; next: WinClawConfig };
  defaultModel: string;
  overrideConfig: WinClawConfig;
  alreadyDefaultConfig: WinClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: WinClawConfig) => config,
      applyDefaultConfig: (config: WinClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: WinClawConfig = { agents: { defaults: {} } };
