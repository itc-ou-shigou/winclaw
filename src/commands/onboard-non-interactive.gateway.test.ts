...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.WINCLAW_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "winclaw");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.WINCLAW_STATE_DIR = stateDir;
    process.env.WINCLAW_CONFIG_PATH = path.join(stateDir, "winclaw.json");
