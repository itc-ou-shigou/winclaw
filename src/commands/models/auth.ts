...
import { readConfigFileSnapshot, type WinClawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: WinClawConfig, model: string): WinClawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("winclaw plugins install")}\`.`,
    );
  }
