...
export async function loadValidConfigOrThrow(): Promise<WinClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: WinClawConfig) => WinClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: WinClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: WinClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("winclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
