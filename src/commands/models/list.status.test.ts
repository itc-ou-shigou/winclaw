...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveWinClawAgentDir: vi.fn().mockReturnValue("/tmp/winclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/winclaw-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveWinClawAgentDir: mocks.resolveWinClawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/winclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveWinClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/winclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/winclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/winclaw-agent-custom");
