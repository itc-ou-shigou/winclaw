...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveWinClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("winclaw models auth setup-token")}\` or \`${formatCliCommand("winclaw configure")}\`.`
          : `Run \`${formatCliCommand("winclaw configure")}\` or set an API key env var.`;
