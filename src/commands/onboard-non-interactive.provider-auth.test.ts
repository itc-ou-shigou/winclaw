...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.WINCLAW_GATEWAY_PASSWORD,
    disableConfigCache: process.env.WINCLAW_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("WINCLAW_STATE_DIR", prev.stateDir);
  restoreEnvVar("WINCLAW_CONFIG_PATH", prev.configPath);
  restoreEnvVar("WINCLAW_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("WINCLAW_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("WINCLAW_SKIP_CRON", prev.skipCron);
  restoreEnvVar("WINCLAW_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("WINCLAW_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("WINCLAW_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("WINCLAW_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.WINCLAW_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "winclaw.json");
  process.env.HOME = tempHome;
  process.env.WINCLAW_STATE_DIR = tempHome;
  process.env.WINCLAW_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("winclaw-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("winclaw-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("winclaw-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("winclaw-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("winclaw-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("winclaw-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
