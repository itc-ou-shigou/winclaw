...
export async function statusCommand(
  ...
  runtime.log(theme.heading("WinClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`winclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("winclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("winclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("winclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("winclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.winclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.winclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("winclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("winclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("winclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("winclaw gateway probe")}`);
  }
}
