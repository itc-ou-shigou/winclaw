...
const requireAgentDir = () => {
  const agentDir = process.env.WINCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("WINCLAW_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under WINCLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-oauth-"));
    process.env.WINCLAW_STATE_DIR = tempStateDir;
    process.env.WINCLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.WINCLAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to WINCLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-minimax-"));
    process.env.WINCLAW_STATE_DIR = tempStateDir;
    process.env.WINCLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.WINCLAW_AGENT_DIR;
