...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under WINCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("winclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-oauth-sync-"));
    ...
    process.env.WINCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-oauth-nosync-"));
    process.env.WINCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside WINCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-oauth-external-"));
    process.env.WINCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* WINCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to WINCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("winclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
