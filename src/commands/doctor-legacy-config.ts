...
export function normalizeCompatibilityConfigValues(cfg: WinClawConfig): {
  config: WinClawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: WinClawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as WinClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as WinClawConfig["browser"],
    };
    changes.push(
