...
function resolveMode(cfg: WinClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("WinClaw doctor");

  const root = await resolveWinClawPackageRoot({
  ...
  let cfg: WinClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("winclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("winclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("winclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("winclaw doctor --fix")}" to apply changes.`);
  }
