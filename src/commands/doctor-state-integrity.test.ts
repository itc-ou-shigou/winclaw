...
type EnvSnapshot = {
  HOME?: string;
  WINCLAW_HOME?: string;
  WINCLAW_STATE_DIR?: string;
  WINCLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    WINCLAW_HOME: process.env.WINCLAW_HOME,
    WINCLAW_STATE_DIR: process.env.WINCLAW_STATE_DIR,
    WINCLAW_OAUTH_DIR: process.env.WINCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: WinClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: WinClawConfig) {
...
async function runStateIntegrityText(cfg: WinClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "winclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.WINCLAW_HOME = tempHome;
    process.env.WINCLAW_STATE_DIR = path.join(tempHome, ".winclaw");
    delete process.env.WINCLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.WINCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when WINCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.WINCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: WinClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints winclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: WinClawConfig = {};
    ...
    expect(text).toMatch(/winclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/winclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /winclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
