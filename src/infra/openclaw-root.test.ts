...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__winclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "winclaw-root");
...
describe("resolveWinClawPackageRoot", () => {
  let resolveWinClawPackageRoot: typeof import("./winclaw-root.js").resolveWinClawPackageRoot;
  let resolveWinClawPackageRootSync: typeof import("./winclaw-root.js").resolveWinClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveWinClawPackageRoot, resolveWinClawPackageRootSync } =
      await import("./winclaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "winclaw");
    const pkgRoot = path.join(project, "node_modules", "winclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "winclaw" }));

    expect(resolveWinClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "winclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "winclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "winclaw" }));

    expect(resolveWinClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveWinClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-winclaw package roots", async () => {
    const pkgRoot = fx("not-winclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-winclaw" }));

    expect(resolveWinClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveWinClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveWinClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
