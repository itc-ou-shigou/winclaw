...
vi.mock("./winclaw-root.js", () => ({
  resolveWinClawPackageRoot: vi.fn(async () => null),
  resolveWinClawPackageRootSync: vi.fn(() => null),
}));
...
let winclawRoot: typeof import("./winclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    winclawRoot = await import("./winclaw-root.js");
  });
  ...
  it("uses resolveWinClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/winclaw");
    (
      winclawRoot.resolveWinClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/winclaw"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "winclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "winclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-winclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/winclaw-bundle");
    (
      winclawRoot.resolveWinClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
