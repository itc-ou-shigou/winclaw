...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["WINCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.WINCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveWinClawPackageRoot } = await import("./winclaw-root.js"));
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveWinClawPackageRoot).mockResolvedValue("/opt/winclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/winclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
