...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.WINCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.WINCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set WINCLAW_APNS_TEAM_ID and WINCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.WINCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.WINCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.WINCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set WINCLAW_APNS_PRIVATE_KEY_P8 or WINCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading WINCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createWinClawPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    winclaw: createWinClawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
