...
export function triggerWinClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.WINCLAW_SYSTEMD_UNIT,
        process.env.WINCLAW_PROFILE,
      );
  ...
  const label =
    process.env.WINCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.WINCLAW_PROFILE);
