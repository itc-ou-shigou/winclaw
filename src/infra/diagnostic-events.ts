...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __winclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__winclawDiagnosticEventsState) {
    globalStore.__winclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__winclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: WinClawConfig): boolean {
