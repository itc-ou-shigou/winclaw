...
import { ensureWinClawCliOnPath } from "./path-env.js";

describe("ensureWinClawCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling winclaw exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-path-"));
    try {
      ...
      const originalFlag = process.env.WINCLAW_PATH_BOOTSTRAPPED;
      ...
      try {
        ...
        if (originalFlag === undefined) {
          ...
          process.env.WINCLAW_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
