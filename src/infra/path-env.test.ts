...
let ensureWinClawCliOnPath: typeof import("./path-env.js").ensureWinClawCliOnPath;

describe("ensureWinClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "WINCLAW_PATH_BOOTSTRAPPED",
    "WINCLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureWinClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling winclaw exists", () => {
    const tmp = abs("/tmp/winclaw-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "winclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.WINCLAW_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.WINCLAW_PATH_BOOTSTRAPPED = "1";
    ensureWinClawCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/winclaw-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "winclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/winclaw-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "winclaw");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "winclaw");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/winclaw-path/case-linuxbrew");
