...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.WINCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*winclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "winclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "winclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "winclaw-gw");
  ...
  records.push(`_winclaw-gw._tcp IN PTR ${instanceLabel}._winclaw-gw._tcp`);
  records.push(`${instanceLabel}._winclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._winclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; winclaw-content-hash: ${contentHash}\n${contentBody}`;
}
