...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "winclaw-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "winclaw-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "winclaw-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "winclaw-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.winclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "winclaw-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers WINCLAW_HOME over HOME", () => {
    vi.stubEnv("WINCLAW_HOME", "/srv/winclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/winclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $WINCLAW_HOME prefix when WINCLAW_HOME is set", () => {
    vi.stubEnv("WINCLAW_HOME", "/srv/winclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/winclaw-home")}/.winclaw/winclaw.json`)).toBe(
      "$WINCLAW_HOME/.winclaw/winclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $WINCLAW_HOME replacement when WINCLAW_HOME is set", () => {
    vi.stubEnv("WINCLAW_HOME", "/srv/winclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/winclaw-home")}/.winclaw/winclaw.json`),
    ).toBe("config: $WINCLAW_HOME/.winclaw/winclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/winclaw")).toBe(path.resolve(os.homedir(), "winclaw"));
  });
  ...
  it("prefers WINCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("WINCLAW_HOME", "/srv/winclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/winclaw")).toBe(path.resolve("/srv/winclaw-home", "winclaw"));
