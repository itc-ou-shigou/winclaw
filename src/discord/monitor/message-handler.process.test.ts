...
const resolveStorePath = vi.fn(() => "/tmp/winclaw-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/winclaw-discord-process-test-sessions.json");
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/winclaw-discord-process-test-sessions.json" },
        channels: {
          discord: {
