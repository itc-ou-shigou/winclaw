...
import type { WinClawConfig, PluginRuntime } from "winclaw/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): WinClawConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
