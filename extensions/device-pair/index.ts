...
import { approveDevicePairing, listDevicePairing } from "winclaw/plugin-sdk";
...
function resolveGatewayPort(cfg: WinClawPluginApi["config"]): number {
  const envRaw =
    process.env.WINCLAW_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: WinClawPluginApi): Promise<string | null> {
