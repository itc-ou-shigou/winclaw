...
import type { WinClawPluginApi } from "winclaw/plugin-sdk/device-pair";
import {
...
} from "winclaw/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: WinClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.WINCLAW_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: WinClawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: WinClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the WinClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the WinClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
