...
export function createToolFactoryHarness(cfg: WinClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<WinClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
