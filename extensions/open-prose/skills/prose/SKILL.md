...
metadata: { "winclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == WinClaw `sessions_spawn`
- **File I/O** == WinClaw `read`/`write`
- **Remote fetch** == WinClaw `web_fetch` (or `exec` with curl when POST is required)
