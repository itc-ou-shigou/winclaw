...
import type { WinClawPluginApi, WinClawPluginService } from "winclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: WinClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: WinClawPluginApi["config"]): string[] {
...
export default function register(api: WinClawPluginApi) {
  ...
  const timerService: WinClawPluginService = {
