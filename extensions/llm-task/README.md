...
`winclaw.invoke --each`) without adding new WinClaw code per workflow.
...
This extension depends on WinClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** WinClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.winclaw/extensions` as a standalone plugin directory.
