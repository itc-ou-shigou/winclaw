...
import type { WinClawPluginApi } from "winclaw/plugin-sdk/diffs";
...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "winclaw-diffs-tool-"));
...
function createApi(): WinClawPluginApi {
  return {
    id: "diffs",
    name: "Diffs",
    description: "Diffs",
    ...
    runtime: {} as WinClawPluginApi["runtime"],
    logger: {
      info() {},
      warn() {},
      error() {},
