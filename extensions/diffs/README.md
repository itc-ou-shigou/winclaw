...
Read-only diff viewer plugin for **WinClaw** agents.
...
Set plugin-wide defaults in `~/.winclaw/winclaw.json`:
...
Before:
WinClaw supports plugins.

After:
WinClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/winclaw-diffs`).
