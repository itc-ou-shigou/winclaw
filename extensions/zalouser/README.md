...
WinClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
winclaw message send --channel zalouser --target <threadId> --message "Hello from WinClaw!"
```
...
```bash
winclaw channels login --channel zalouser              # Login via QR
winclaw channels login --channel zalouser --account work
winclaw channels status --probe
winclaw channels logout --channel zalouser
```
...
winclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
winclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
winclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `winclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
winclaw channels login --channel zalouser --account work
winclaw message send --channel zalouser --account work --target <id> --message "Hello"
