...
function resolveWinClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.WINCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set WINCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveWinClawRoot(), "dist", "extensionAPI.js");
