/**
 * Twitch channel plugin for WinClaw.
 *
...
import { buildChannelConfigSchema } from "winclaw/plugin-sdk/twitch";
 ...
 * for WinClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: WinClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: WinClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: WinClawConfig): boolean => {
