...
import {
  ...
  type WizardPrompter,
} from "winclaw/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: WINCLAW_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: WinClawConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var WINCLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.WINCLAW_TWITCH_ACCESS_TOKEN?.trim();
